...
/**
 * Validar archivo sea Pdf
 */
function fileValidator(container){
    var nombre = String(document.getElementById('cupon').value);
    var regex=/^.+\.([pP][dD][fF])$/;
    if (!regex.test(nombre)){
        alert(("Cupon invalido, por favor ingrese pdf"));
        return false;
    }
    return true; 
}
 ...
 */
 ...
 function dataValidator() {
     ...
     if (fileValidator(container) && nameValidator(container) && phoneValidator(container)) {
         return true;
     }
     return false;

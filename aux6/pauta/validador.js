...
function validacionLogin() {
    ...
    let regex = /^[a-zA-Z ]*$/;
    if (nombre.length > 10 || nombre.length < 4 || !regex.test(nombre)) {
        msgError += 'Username no valido ';
    }
    ...
    if (apikey.length !== 14) {
        msgError += 'API_KEY no valida'
    }

    mostrarError(msgError);
    return msgError === ''; // retorna true entonces se envia el formulario
